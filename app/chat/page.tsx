...
const UI_TEXT = {
  en: {
    ...
    deleteConfirm: 'Are you sure you wish to delete this article?',
  },
  hi: {
    title: 'लोकसेवा मित्र',
    ...
    deleteConfirm: 'क्या आप इस लेख को हटाना चाहते हैं?',
  },
  mr: {
    title: 'लोकसेवा मित्र',
    ...
    deleteConfirm: 'तुम्ही हे लेख हटवू इच्छिता का?',
  },
};
  ...
  return (
    <div className="flex h-screen bg-[#faf8f5] text-zinc-950 overflow-hidden">
      ...
      <motion.aside
        ...
        className="bg-white flex flex-col shrink-0 overflow-hidden text-zinc-950 z-30 shadow-none border-r-2 border-zinc-900"
      >
        <div className="p-6 border-b-2 border-zinc-900 flex flex-col gap-3 whitespace-nowrap">
          ...
          <button
            ...
            className="w-full bg-zinc-900 hover:bg-zinc-700 text-white p-3 text-xs font-black tracking-widest uppercase transition-all"
          >
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2 w-[280px]">
          {sessions.map((s) => (
            <motion.div
              ...
              className={`group p-4 cursor-pointer flex justify-between items-center text-sm transition-all border ${
                activeId === s.id ? 'bg-zinc-100 border-zinc-900' : 'hover:bg-zinc-50 border-transparent text-zinc-700'
              }`}
            >
              <span className="truncate w-44 font-medium">{s.title}</span>
              <button
                onClick={(e) => deleteSession(s.id, e)}
                className="opacity-0 group-hover:opacity-100 text-rose-950 hover:text-red-600 font-black text-lg"
              >
                ×
              </button>
            </motion.div>
        ...
        <div className="p-4 border-t-2 border-zinc-900 space-y-2 text-xs">
          <Link href="/dashboard" className="block p-2 hover:bg-zinc-100 font-mono font-bold">
            ← Dashboard
          </Link>
          <button className="block w-full text-left p-2 hover:bg-zinc-100 font-mono font-bold text-rose-950">
            Logout
          </button>
        </div>
      </motion.aside>
        ...
        <header className="bg-white p-4 shrink-0 z-20 border-b-4 border-double border-zinc-900">
          <div className="flex justify-between items-center border-b border-zinc-400 pb-3 mb-3 text-xs font-mono font-bold text-zinc-600 uppercase tracking-wider">
            ...
            <select
              ...
              className="bg-zinc-100 text-zinc-800 text-xs font-bold px-2 py-1 outline-none border border-zinc-300"
            >
          ...
          <div className="flex items-center gap-6">
            <button
              ...
              className="p-2 hover:bg-zinc-100 text-2xl border border-zinc-300 shadow-inner"
            >
              {isSidebarOpen ? '⇠' : '⇢'}
            </button>
            ...
            {activeSession && activeSession.messages.length > 0 && (
              <button
                onClick={downloadPDF}
                className="bg-white border-2 border-rose-950 hover:bg-rose-50 text-rose-950 px-4 py-3 text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all"
              >
                {UI_TEXT[lang].pdf}
              </button>
            )}
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-10 space-y-8 bg-[#faf8f5]">
          <AnimatePresence>
            {activeSession?.messages.length === 0 ? (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-full flex items-center justify-center text-center">
                <div className="bg-white p-16 shadow-none border-2 border-zinc-900 max-w-lg text-left relative">
                  <span className="absolute -top-6 -left-6 text-7xl font-serif font-black text-rose-950 leading-none">
                    "
                  </span>
              ...
              activeSession?.messages.map((msg, idx) => (
                ...
                >
                  <div
                    className={`max-w-[85%] md:max-w-[75%] p-6 border-2 ${
                      msg.role === 'user'
                  ...
                  >
                    <div
                      className={`text-[10px] font-mono font-black uppercase tracking-widest mb-3 pb-1 border-b ${
                        msg.role === 'user' ? 'border-rose-300' : 'border-zinc-300'
                      }`}
                    >
          ...
          {isLoading && (
            <div className="text-xs text-rose-950 font-mono font-black animate-pulse uppercase tracking-widest flex items-center gap-2">
              <span className="w-2 h-2 bg-rose-950 rotate-45"></span>
        ...
        <div className="bg-white border-t-2 border-zinc-900 p-6 shadow-none">
          <form onSubmit={handleSend} className="max-w-4xl mx-auto flex gap-4">
            ...
            <button
              ...
              className="bg-zinc-950 hover:bg-zinc-700 disabled:bg-zinc-400 text-white px-8 py-4 font-black disabled:opacity-50 tracking-widest text-xs uppercase transition-all active:scale-95"
            >
              {UI_TEXT[lang].send}
            </button>
          </form>
